...
def Batch():
    ...
    tInputs = torch.from_numpy(sequences)
...
class recurrent_model(nn.Module):
    ...
    def initHidden(self,batch_size):
        ...
        if lstm:
            ...
            hiddens.append( torch.zeros(batch_size, self.hidden_size) )
            hiddens.append( torch.zeros(batch_size, self.hidden_size) )
    ...
    def extract_grad(self):
        ...
        for param in self.parameters():
            if param.grad is not None:
                grad_numpy = param.grad.detach().cpu().numpy().flatten()
                sz = grad_numpy.shape[0]
                pvec[count:count + sz] = grad_numpy
                count += sz
        return pvec.copy()
    ...
    def extract_parameters(self):
        ...
        count = 0
        for param in self.parameters():
            param_numpy = param.detach().cpu().numpy().flatten()
            sz = param_numpy.shape[0]
            pvec[count:count + sz] = param_numpy
            count += sz
        return pvec.copy()
    ...
    def reshape_parameters(self,pvec):
        ...
        for name,param in self.named_parameters():
            ...
            reshaped = raw.reshape(param_shape)
    ...
    def inject_parameters(self, pvec):
        ...
        with torch.no_grad():
            for param in self.parameters():
                ...
                raw = pvec[count:count + sz]
                reshaped = raw.reshape(param_shape)
                param.copy_(torch.from_numpy(reshaped))
                count += sz
...
def test_model(rnn):
    ...
    _d = output.max(1)[1].detach().cpu().numpy()
    _l = _lab.detach().cpu().numpy()
    correct = (_d==_l).sum()

    return float(correct)/(_d).shape[0],loss.item()
...
def train(rnn,_inp,_len,_lab,opt=True):
    ...
    seq_length = _len[0].item() if _len.numel() > 0 else 0
...
if __name__=='__main__':
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if epoch % print_every == 0:
            ...
            print('%d %d%% (%s) %.4f %s / %s %s' % (epoch, float(epoch) / n_epochs * 100, timeSince(start), loss.item(), line, guess, correct))
            print(test_model(rnn))
